...
export class LoginFormStore extends Submittable<Field> {
  ...
  @action login = (): void => {
    if (this.fields.get("username").value === "editor" && this.fields.get("password").value === "pass") {
      this.fakeLogin({
        "id": 16,
    ...
    } else if (this.fields.get("username").value === "writer" && this.fields.get("password").value === "pass") {
      this.fakeLogin({
        "id": 17,
