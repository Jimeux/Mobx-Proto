...
export class TicketListEditor extends Component<TicketListProps, {}> {
  ...
  componentDidMount() {
    this.props.ticketStore.fetchTickets(TicketStatus.Editing)
  }
  ...
  render() {
    ...
    const tickets = currentTickets.map(ticket =>
      <TicketRow key={`ticket-${ticket.id}`}
                 ticket={ticket}
                 onClick={() => this.props.appStore.setPath(`/editor/articles/view/${ticket.id}`)}/>)
  ...
  renderActionMenu = () =>
    <div className="action-menu">
      ...
      <div className="actions">
        ...
        <i className="material-icons" onClick={() => this.props.ticketStore.fetchTickets(TicketStatus.Editing)}>
          refresh
        </i>
      </div>
    </div>
